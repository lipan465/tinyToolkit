...
class basic_printf_context :
 ...
 public:
  /**
   ...
   \endrst
   */
  basic_printf_context(OutputIt t_out, basic_string_view<char_type> format_str,
                       basic_format_args<basic_printf_context> args)
      : base(t_out, format_str, args) {}
...
OutputIt basic_printf_context<OutputIt, Char, AF>::format() {
  ...
  while (it != end) {
    ...
    if (c != '%') continue;
    if (it != end && *it == c) {
      t_out = std::copy(start, it, t_out);
      start = ++it;
      continue;
    }
    t_out = std::copy(start, it - 1, t_out);
    ...
    // Format argument.
    visit_format_arg(AF(t_out, spec, *this), arg);
  }
  return std::copy(start, it, t_out);
}
