...
template <class Char> class formatbuf : public std::basic_streambuf<Char> {
 ...
 public:
  formatbuf(basic_buffer<Char>& t_buffer) : buffer_(t_buffer) {}
...
void write(std::basic_ostream<Char>& os, basic_buffer<Char>& buf) {
  ...
  do {
    ...
    os.write(t_data, static_cast<std::streamsize>(n));
    t_data += n;
    size -= n;
  } while (size != 0);
}
...
void format_value(basic_buffer<Char>& t_buffer, const T& value) {
  internal::formatbuf<Char> format_buf(t_buffer);
  std::basic_ostream<Char> output(&format_buf);
  output.exceptions(std::ios_base::failbit | std::ios_base::badbit);
  output << value;
  t_buffer.resize(t_buffer.size());
}
...
struct fallback_formatter<
  ...
  auto format(const T& value, Context& ctx) -> decltype(ctx.out()) {
    basic_memory_buffer<Char> t_buffer;
    internal::format_value(t_buffer, value);
    basic_string_view<Char> str(t_buffer.data(), t_buffer.size());
