...
namespace tinyToolkit
{
	 ...
	 *
	 */
	void HMAC_SHA256::Append(const char * value)
	{
		if (value == nullptr)
		{
			return;
	 ...
	 *
	 */
	void HMAC_SHA256::Append(const uint8_t * value)
	{
		if (value == nullptr)
		{
			return;
	 ...
	 *
	 */
	void HMAC_SHA256::Append(const std::string & value)
	{
	 ...
	 *
	 */
	void HMAC_SHA256::Append(const char * value, std::size_t length)
	{
	 ...
	 *
	 */
	void HMAC_SHA256::Append(const uint8_t * value, std::size_t length)
	{
		if (value == nullptr || length == 0)
		{
			return;
	 ...
	 *
	 */
	void HMAC_SHA256::Append(const std::string & value, std::size_t size)
	{
	...
	void HMAC_SHA256::Generate()
	{
		if (!_isComputed)
		{
			...
			sha256.Append(_sha256.Digest(), SHA256_DIGEST_SIZE);

			memcpy(_digest, sha256.Digest(), SHA256_DIGEST_SIZE);
