...
namespace tinyToolkit
{
	namespace crypto
	{
		...
		void SHA224::Final(Context & context, uint8_t * digest)
		{
			...
			uint32_t blockNumber = (1 + ((TINY_TOOLKIT_SHA224_BLOCK_SIZE - 9) < (context.blockLength % TINY_TOOLKIT_SHA224_BLOCK_SIZE) ? 1 : 0));
		...
		{
			uint32_t fill = TINY_TOOLKIT_SHA224_BLOCK_SIZE - context.blockLength;
			...
			if ((context.blockLength + length) < TINY_TOOLKIT_SHA224_BLOCK_SIZE)
			{
			...
			uint32_t blockNumber = overLength / TINY_TOOLKIT_SHA224_BLOCK_SIZE;
			...
			fill = overLength % TINY_TOOLKIT_SHA224_BLOCK_SIZE;
