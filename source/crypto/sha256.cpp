...
namespace tinyToolkit
{
	namespace crypto
	{
		 ...
		 *
		 */
		void SHA256::Append(const char * content)
		{
			if (content == nullptr)
			{
				return;
		 ...
		 *
		 */
		void SHA256::Append(const uint8_t * content)
		{
			if (content == nullptr)
			{
				return;
		 ...
		 *
		 */
		void SHA256::Append(const std::string & content)
		{
		 ...
		 *
		 */
		void SHA256::Append(const char * content, std::size_t length)
		{
		 ...
		 *
		 */
		void SHA256::Append(const uint8_t * content, std::size_t length)
		{
			if (content == nullptr || length == 0)
			{
				return;
		 ...
		 *
		 */
		void SHA256::Append(const std::string & content, std::size_t length)
		{
			if (length == 0)
			{
				return;
		...
		void SHA256::Final(Context & context, uint8_t * digest)
		{
			...
			uint32_t blockNumber = (1 + ((TINY_TOOLKIT_SHA256_BLOCK_SIZE - 9) < (context.blockLength % TINY_TOOLKIT_SHA256_BLOCK_SIZE) ? 1: 0));
		 ...
		 *
		 */
		void SHA256::Update(Context & context, const uint8_t * content, uint32_t length)
		{
			uint32_t fill = TINY_TOOLKIT_SHA256_BLOCK_SIZE - context.blockLength;
			...
			if (context.blockLength + length < TINY_TOOLKIT_SHA256_BLOCK_SIZE)
			{
			...
			uint32_t blockNumber = overLength / TINY_TOOLKIT_SHA256_BLOCK_SIZE;
			...
			fill = overLength % TINY_TOOLKIT_SHA256_BLOCK_SIZE;
