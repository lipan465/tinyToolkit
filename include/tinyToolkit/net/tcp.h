...
namespace tinyToolkit
{
	...
	public:
		 ...
		 *
		 */
		TCPClientPipe(int32_t epoll, int32_t socket, ITCPSession * session, EVENT_TYPE type) : _epoll(epoll),
																							   _socket(socket),
																							   _session(session)
		{
	...
	public:
		 ...
		 *
		 */
		TCPServerPipe(int32_t epoll, int32_t socket, ITCPServer * server, EVENT_TYPE type) : _epoll(epoll),
																							 _socket(socket),
																							 _server(server)
		{
			eventValue.type = type;
