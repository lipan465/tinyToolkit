...
template <typename Char>
inline void reset_color(basic_memory_buffer<Char>& t_buffer) FMT_NOEXCEPT {
  const char* begin = data::RESET_COLOR;
  const char* end = begin + sizeof(data::RESET_COLOR) - 1;
  t_buffer.append(begin, end);
}
...
std::basic_string<Char> vformat(
  ...
  if (ts.has_emphasis()) {
    ...
    t_buffer.append(escape.begin(), escape.end());
  }
  ...
  internal::vformat_to(t_buffer, format_str, args);
  if (has_style) {
    reset_color<Char>(t_buffer);
  }
  return fmt::to_string(t_buffer);
}
}  // namespace internal
