...
template <typename Char> struct format_part {
 public:
  struct named_argument_id {
    FMT_CONSTEXPR named_argument_id(internal::string_view_metadata t_id)
        : id(t_id) {}
...
class prepared_format {
 public:
  ...
  std::basic_string<char_type> format(const Args&... args) const {
    basic_memory_buffer<char_type> t_buffer;
    typedef back_insert_range<internal::basic_buffer<char_type>> range;
    this->vformat_to(range(t_buffer), *checked_args(format_, args...));
    return to_string(t_buffer);
  }
  ...
  format_to(std::back_insert_iterator<Container> out,
    ...
    internal::container_buffer<Container> t_buffer(internal::get_container(out));
    typedef back_insert_range<internal::basic_buffer<char_type>> range;
    this->vformat_to(range(t_buffer), *checked_args(format_, args...));
    return out;
  }
